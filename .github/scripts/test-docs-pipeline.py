...
class HeroTests(unittest.TestCase):
    def test_hero_h1_regex_demotes_to_styled_paragraph(self):
        out = b.HERO_H1_RE.sub(r'<p class="hero-title"\1>\2</p>', "  <h1>declscope</h1>\n")
        self.assertEqual(out, '  <p class="hero-title">declscope</p>\n')  # no heading -> no toc id


class RewriteTests(unittest.TestCase):
    def test_readme_anchor_and_docs_prefix_rewrites(self):
        amap = {"the-core-namespace": "namespaces.md"}
        errors: list[str] = []
        out = b.rewrite_links(
            "See [core](#the-core-namespace) and [x](docs/x.md#some-heading).\n",
            amap,
            from_readme=True,
            err=errors.append,
        )
        self.assertIn("(namespaces.md#the-core-namespace)", out)
        self.assertIn("(x.md#some-heading)", out)
        self.assertEqual(errors, [])

    def test_docs_asset_link_is_not_moved(self):
        # Assets under docs/ are copied to the same path, so only page links move.
        errors: list[str] = []
        out = b.rewrite_links("![b](docs/boundaries.png) [p](docs/x.md)\n", {}, from_readme=True, err=errors.append)
        self.assertIn("(docs/boundaries.png)", out)
        self.assertIn("(x.md)", out)
        self.assertEqual(errors, [])

    def test_repo_file_link_goes_to_github(self):
        errors: list[str] = []
        out = b.rewrite_links("[s](skills/declscope-adoption/SKILL.md#x)\n", {}, from_readme=True, err=errors.append)
        self.assertIn("](https://github.com/mpyw/declscope/blob/main/skills/declscope-adoption/SKILL.md#x)", out)
        self.assertEqual(errors, [])

    def test_missing_repo_file_link_is_an_error(self):
        errors: list[str] = []
        b.rewrite_links("[s](no/such/file.go)\n", {}, from_readme=True, err=errors.append)
        self.assertTrue(any("does not exist: no/such/file.go" in e for e in errors))
    ...
    def test_subpage_local_asset_src_gets_dotdot_prefix(self):
        errors: list[str] = []
        out = b.rewrite_links(
            '<img src="demo/x.gif"> <a href="https://z"> <img src="/root.png"> [md](docs/y.md)\n',
            {"": "index.md"},
            from_readme=True,
            err=errors.append,
            sub_page=True,
...
class ResolveTests(unittest.TestCase):
    ...
    def test_same_site_absolute_and_qualified(self):
        ...
        self.assertEqual(resolve("index.html", "/declscope/aws/", existing), "aws/index.html")
        self.assertEqual(resolve("index.html", "https://mpyw.github.io/declscope/aws/", existing), "aws/index.html")
