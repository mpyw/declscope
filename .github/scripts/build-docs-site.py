...
would break the many intra-README anchor links (`#the-core-namespace`, …) and
...
# site at the same path so those links resolve. Each entry is a path relative to
# the repo root. The logos are also the header logo and favicon (mkdocs.yml
# theme.logo/favicon), and social-preview.png is the OG/Twitter card image wired
# in the theme override (mkdocs.yml theme.custom_dir), which the README never
# links. social-preview.html, the page that image is captured from, is left out.
ASSETS = [
    "docs/boundaries.png",
...
# Same-group entries stay contiguous, so literate-nav renders one parent; within
# a group they follow this dict's key order, not the alphabetical discovery order.
DOC_NAV: dict[str, dict[str, str]] = {}

# A README link to a repository file the site does not carry (neither a docs/
# page nor an ASSET) points at that file on GitHub instead. The target must
# exist, so a renamed file fails the build rather than a dead link shipping.
REPO_BLOB = "https://github.com/mpyw/declscope/blob/main/"
# A relative Markdown link target: not a URL, not root-absolute, not an anchor.
REPO_FILE_LINK_RE = re.compile(r"\]\((?![a-z][a-z0-9+.-]*:|/|#)([^)\s#]+)((?:#[^)\s]*)?)\)", re.IGNORECASE)
...
# "docs/x.png" would 404. Prefix such refs with ../ on sub-pages (the home page
...
def carried_by_site(path: str) -> bool:
    """Whether a README-relative path is served by the site itself: a docs/*.md
    page (flattened to the root) or something under an ASSETS entry."""
    if path.startswith("docs/") and path.endswith(".md"):
        return True
    return any(path == a or path.startswith(a.rstrip("/") + "/") for a in ASSETS)


def repo_file_link(m: re.Match, err) -> str:
    """Point a README link at a repository file the site does not carry to that
    file on GitHub; anything the site serves is left for MkDocs to resolve."""
    path, frag = m.group(1), m.group(2)
    if carried_by_site(path):
        return m.group(0)
    if not (REPO / path).exists():
        err(f"link to a repository file that does not exist: {path}")
        return m.group(0)
    return f"]({REPO_BLOB}{path}{frag})"
    ...
    *,
    ...
    def rewrite_prose(text: str) -> str:
        ...
        if from_readme:
            ...
            # Links to repository files the site does not carry go to GitHub. This
            # runs first: it must see the README's own spelling of each path.
            text = REPO_FILE_LINK_RE.sub(lambda m: repo_file_link(m, err), text)
            # README lives at the repo root; docs/*.md pages become root-level
            # siblings. Only page links move: an asset under docs/ is copied to
            # the same path (ASSETS), so its link already resolves.
            text = re.sub(r"\]\(docs/([^)#]+\.md)((?:#[^)]*)?)\)", r"](\1\2)", text)
...
def main() -> int:
    ...
    # Within a group, order docs by DOC_NAV declaration,
