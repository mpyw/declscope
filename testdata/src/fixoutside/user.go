...
//
//declscope:package
func aux() int { return 2 } // want `func aux does not carry namespace "user" anywhere in its name; rename it to userAux`

// Every use is in a collected file, so this one is renamed.
func local() int { return 3 } // want `func local does not carry namespace "user" anywhere in its name; rename it to userLocal`
