...
var count = 10 // want `var count does not carry namespace "user" anywhere in its name; rename it to userCount`

func Add(userCount int) int { return userCount + count }

// Bound at a reference by a local declared before it.
var limit = 3 // want `var limit does not carry namespace "user" anywhere in its name; rename it to userLimit`
...
var total = 7 // want `var total does not carry namespace "user" anywhere in its name; rename it to userTotal`
...
var size = 1 // want `var size does not carry namespace "user" anywhere in its name; rename it to userSize`
