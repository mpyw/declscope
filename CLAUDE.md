...
`rules.naming.qualify` reads an `internal.Mode` — `Always`, `Never` or `OnDemand` — and `Mode.Applies` is the predicate it gates on. **It defaults to `never`: the naming rule is opt-in.** It shipped as `ondemand` first, and measurement rejected that. Naming findings did not correlate with encapsulation problems (measured on the containment rule at v0.1: goroutinectx 0 boundary vs 22 naming, gormreuse 11 vs 38, zerologlintctx 3 vs 20, suve 749 vs 650), whether a prefixed name reads as a noun phrase or an imperative depends on the file name's part of speech, which the tool cannot see (`collectAddFunc` vs `commentsAttached`), and the containment test leans on English spelling. Do not make it the default again on the strength of any one repository. Under `ondemand` it requires the mark only once a package has a second namespace; in a package with one, every other rule is structurally inert anyway. Mode declares the values it accepts as an `internal.ModeSet`, so a rejected value is answered with what that setting accepts, and `Mode.String()` returns the same spelling the config takes.
...
`internal.SurplusMode` is a sibling of `internal.Mode` rather than three more `Mode` values. `Mode` carries `Applies(namespaces)`, a question about how many namespaces a package has, which means nothing for this rule, and a `Mode` that could hold `loose` would still need an answer to it. The shape is the same (`SurplusModeSet.Parse`, the accepted-values error, `String()` round-tripping the config spelling), and the part the two share, spelling "a, b or c", is `modeJoin`.
