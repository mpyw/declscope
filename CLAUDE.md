...
Config files do **not** compose. `findUp` returns the first file it finds, so the nearest one owns every key and the one above is never read. This is not what a `.gitignore` does, and the difference is pinned by `TestNestedConfigReplacesTheOneAbove` rather than left to a sentence. Making them accumulate is a change to how configs compose and would raise the same question for `rules` and `defaults`; do not slip it in under a `filter` change.
