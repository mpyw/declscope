...
**A `//declscope:namespace` that restates the name the file derives is not reported.** The check would be sound. `namespace.Of` reads no configuration, so the comparison cannot be flipped by a line of YAML. It stays out for the other half of the inert test's reason. Such a directive may be pinning the namespace against a future file rename, and it is the only way an author can say that. Reporting it would ask the author to delete the thing doing that job. The only escape would be `//declscope:ignore directive` on the whole file, which silences more than the one report. This is the close of #99.
...
internal/
  ...
  rule/                   the rule vocabulary, shared by diagnostics, config, baseline and ignores.
                          ...
                          SurplusMode and DirectiveMode, the vocabularies each rule's
...
| `directive` | A directive that binds nothing, or that is malformed or misplaced. `rules.directive: strict` also reports a scope directive naming the scope everything it reaches would have without it under the configuration in force | None under `loose`. Under `strict`, delete the redundant directive |
...
- **It is withheld when everything the enclosing directive decides for would be narrowed.** The directive would then bind nothing, the directive rule would report what `-fix` had just written, and that rule has no fix. The case is real: an exported type whose directive exists only to widen unexported fields nobody reads. The advice there is to delete the directive, and the dependents are reported without a fix. A directive that already binds nothing is not a reason to withhold.
...
### `rules.directive: strict`

`strict` makes that trade on purpose, for a repository that wants every directive to change something today. **It is opt-in and `loose` is the default** for the reason `surplus` gives: an upgrade must not add reports to a repository whose config did not change, and under `strict` a change of `defaults.unexported` reports every directive restating the new default. There is no `off`: the `directive` rule also carries malformed, unknown and conflicting directives, and a switch that silenced those along with the unused ones is the key that silences more than its name says. `//declscope:ignore directive` answers one case. `spec/directive_strict.fsl` is the model.
...
- **It judges everything in reach, shadowed or not.** A nearer directive is judged against this one's scope today and against the level beyond once this one is deleted. The two must agree, or the deletion would flip the nearer one's judgment. `directivestrict/a.go`'s `mixed` and the convergence cases pin it.
- **The chain puts back what a merge hid, for both modes.** `Decl.Merge` keeps the block's directive in `Beneath` when a spec states its own, and `bindAtScopeSite` walks it. Without it, a spec's `private` under a `package` block is judged against the file and called inert, and deleting it widens the spec. `directiveloose/b.go` and `c.go` pin both directions.
...
- **The fix is withheld where the deletion would move another report**, with the report kept: a taker of the directive's scope crossing (the boundary message names the level that decided), a field whose type the boundary fix widens in this run (`widened`, passed from `report`), a `//declscope:package` restating an enclosing one while `surplus` reads it (the declaration would join that directive's dependents, which `surplus: strict` judges), one under an ignore covering `surplus`, and one whose next level out is a directive that keeps its own report. That last one is a block: a spec joining it changes the specs its report names (`fixdirectivestrictwithheld/d.go`), or, where `loose` alone reports it, turns "states its own scope" into a named report (`f.go`). An outer directive that an ignore answers is judged like any other. If it is redundant it stays redundant and answered after gaining the declaration; if it is not, `decide` refuses it, which keeps a bare ignore from being left answering nothing.
- **Only a pass that sees every file offers the fix**, like a rename. The ordinary variant of a package with in-package tests cannot see a crossing from a test file, and the driver applies what either variant offers, so the deletion the test variant withholds would be made anyway (`c.unseen(pass).all`). A file the build excludes is read for its names alone, so a taker it names counts as crossing. `spec/directive_strict.fsl` models both as `blind`.
- Every withholding reason has an in-process case in `fixdirectivestrictwithheld`, `fixdirectivestrictsurplus` or `TestSuggestedFixDirectiveStrictUnseenTests` that fails when the reason is removed, and a convergence case that shows the report the fix would have changed.

declscope holds itself to `directive: strict` in `.declscope-strict.yaml`. Adopting it deleted six `//declscope:private` directives on unexported types in `internal/` (`ignoreSite`, `renameState`, `scopeSite`, `surplusDeclaration`, `surplusState`, and `scopesiteRemovals`). Each restated the default, and each type's fields that cross say so with their own `//declscope:package`, which binds with or without the type's.
...
- `testdata/src/directiveloose` pins that the default keeps a directive naming today's default, and `directivestrict` and `directivestrictpkg` pin `strict` under each default. `directivestrict` states `surplus: off`, since surplus would report the `//declscope:package` directives it keeps. `fixdirectivestrict/b.go` takes no edit and has no golden on purpose: it is the withheld crossing case. `fixdirectivestrictwithheld` (under `surplus: off`) and `fixdirectivestrictsurplus` (under `defaults.unexported: package`) hold one withholding reason per file, and only the files a fix edits have goldens. `directivestricttests` is read by `TestSuggestedFixDirectiveStrictUnseenTests` rather than through `want` comments, since its boundary report exists in the test variant alone. The goldens compare after gofmt, so the bytes the deletion leaves (the `//` separator, blank lines, CRLF) are pinned by `TestScopesiteRemoval` in `internal/scopesite_test.go`.
