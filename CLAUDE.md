...
The rules were `escape`, `promote` and `demote` until #21. `escape` is Go's own word for a value escaping to the heap, and the documentation had always called the thing a *boundary crossing*; `promote`/`demote` matched neither the code implementing them (`namespace.Qualify`/`Unqualify`) nor the job those words are wanted for (promoting a namespace to a real package, #22). Nothing accepts the old spellings, and nothing should: the rename happened before the first release, so no configuration or baseline anywhere was ever written with them. Migration machinery was written for it and then removed — a reminder that "what about existing users" is not a question an unreleased tool gets to ask.
