...
`rules.qualify` is tri-state (`internal.QualifyMode`) and defaults to `ondemand`, requiring it only once a package has a second namespace — in a package with one, every other rule is structurally inert anyway, since every reference is already inside the single namespace. The documented spellings are `always`, `never` and `ondemand`, one enum rather than two booleans and a string; `true` and `false` stay accepted as aliases, which is why `ParseQualifyMode` takes both a bool and a string, and `QualifyMode.String()` returns the documented spelling so nothing printed points at an alias.
...
`internal/rule` holds the one vocabulary: `boundary`, `qualify`, `unqualify`. The same name is the diagnostic's `Category`, the `Rule` field of a baseline key, and what an ignore directive targets. **Adding a rule means adding it there**, not inventing a string at the report site.

The rules were `escape`, `promote` and `demote` until #21. `escape` is Go's own word for a value escaping to the heap, and the documentation had always called the thing a *boundary crossing*; `promote`/`demote` matched neither the code that implements them (`namespace.Qualify`/`Unqualify`) nor the job those words are wanted for (promoting a namespace to a real package, #22). **The old spellings are refused, never aliased**, in all three places a rule name is read, from one table (`rule.Renamed`): a `rules.promote`/`rules.demote` config key, a baseline section, and an ignore target each fail with a message naming the new spelling. An alias would have put two names on one rule — the thing "one vocabulary" exists to prevent — and would have kept `promote`/`demote` half-occupied. Silently ignoring them was never available: unknown config keys are a hard error by design, and a baseline section that matches nothing resurfaces every violation it recorded with nothing saying why (that is what an unrecognised section still does, which is why the former names are caught by name). The baseline error points at regeneration, the documented remedy for every baseline problem, which never reads the file it replaces. The `File.Rules.Promote`/`Demote` fields in `internal/config` exist only so that the decoder, which rejects unknown keys, does not get there first with a message that names nothing.
...
A `foreign-method` rule existed briefly and was removed. `checkBoundary` reports a method at its *declaration*, not at the call, so a method grown on another namespace's type is already caught wherever it is used — the declaration-site rule only ever added a second diagnostic at the same position. The one case it covered alone was a foreign method that is never called, which is dead code and an unused-code linter's business, the same reasoning that kept "exported but unused outside the package" out of scope.

`namespace.Unqualify` (unqualify's rename) lowers a leftover initialism the way Go spells one (`userID` → `id`, `userURLPath` → `urlPath`), which the naive version got wrong (`iD`). It returns a second value explaining any refusal.

`checkUnqualify` exempts a name identical to its namespace. The causality usually runs the other way there — `user.go` is named after the `user` it declares — so there is no label to strip, and the only advice available would have been "rename it by hand". `qualify` still accepts such a name.

**Not being able to derive a rename is never a reason to stay silent.** `checkUnqualify` gates on `namespace.HasPrefix` — whether there is a label at all — and then reports either way, embedding `Unqualify`'s reason when it has no suggestion. An earlier version skipped the declaration entirely, which left a codebase half-converted under `unqualify: true` with nothing saying why. `checkQualify` has always behaved this way when its rename target is taken; keep new rules consistent with it.
...
//declscope:ignore unqualify     // silence named rules only (boundary, qualify, unqualify)
...
**Accounting is per physical directive, never per target.** `Decl.Merge` copies a block's ignore into every spec, and one `dir` is shared by every name in `var a, b` and `x, y int`, so a per-target "used" flag judged one comment N times and reported it unused whenever any sibling did not need it (#10). `collection.ignores` keys an `ignoreSite` by the directive's position; `add` names every target the directive reaches on it, `ignored` marks the site used, and `reportUnusedIgnores` reports each unused site once, listing the declarations it was written for. Every declaration-level comment group goes through `collection.parseDecl`, which registers the site and records problems once per comment (a bogus directive on a block used to be reported once per spec), and marks the group **consumed** — see below.

**Only a pass that sees every reference may call a directive unused.** The ordinary variant of a package with in-package `_test.go` files cannot see the references those files make, so a directive needed only by a test was reported unused there and reported necessary by the test variant (#11). `reportUnusedIgnores` returns early when `hasUnseenTests` holds — the same predicate that withholds renames, kept on `collection` because both features ask it — and defers to the test variant, on exactly the reasoning that keeps unmatched baseline entries unreported (below). The consequence, documented in the README, is that `-test=false` yields no unused-ignore report for such packages. A more permissive rule — report in the ordinary variant unless the test variant *could* need it — would have meant parsing the sibling test files for references, which is the heuristic path the rename guard already rejected.

**A directive that binds to nothing is reported as misplaced, never dropped.** `type X struct { //declscope:ignore boundary` was neither applied nor reported (#12): go/parser attaches that comment to nothing, and `lineComments` was consulted only for `ast.FuncDecl`. The fix is both halves of the contract. Binding: `looseTrailing` finds an unattached comment trailing a node's **first or last line**, and `addFunc`, `addGenDecl` (for the block's own `(`/`)` lines) and `specGroups` read it; `attached` lists the groups the parser did hang on something inside the node, which win, so a field's trailing comment on the brace line stays the field's — and `specGroups` parses only the spec's *own* Doc/Comment plus the loose groups, never the fields' (an earlier draft fed the fields' groups into the type's parse and the convergence test caught the field's `//declscope:package` landing on the type). Reporting: `collection.stray` walks every comment group after the package clause that `parseDecl` never consumed and reports each directive in it through `directive.Stray`. The first-or-last-line rule is deliberately blunt: a directive on a middle line of a multi-line signature or struct is misplaced, because the alternative was guessing which declaration a comment "near" one belongs to.
