...
`rules.allowBoundary` is not that shape. It switches one rule, the way `rules.allowSurplus` does, and it exists for the repository that wants the ownership mark in a name without the scope behind it. **It must not be made to switch `surplus` as well.** Turning boundary off does leave `surplus` auditing directives that no longer do a job, and the README says to set both, but coupling them in code is exactly the key that silences more than its name says. Two keys, each doing what it is called.
...
| `surplus` | A `//declscope:package` with no use from another namespace visible to declscope. **On by default**; `rules.allowSurplus: true` turns it off | None — a false positive would be an automatic edit deleting a load-bearing directive |
| `directive` | A directive that binds nothing, or that is malformed or misplaced | None |

The config key is `rules.allowSurplus`, not `rules.surplus`, which is the one place a rule's name and its configuration key differ. `surplus: true` would have read as "surplus: yes please", the opposite of what it does. The rule name still spells the diagnostic category, the baseline key and the `//declscope:ignore` target, and writing that ignore is rare enough that the mismatch costs little. Default on was measured rather than assumed: across eight repositories, including declscope itself with 43 directives, turning it on changes no diagnostic count.
