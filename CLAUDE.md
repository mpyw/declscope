...
`namespace.Contains` is what the rule gates on: the namespace may sit anywhere in the name, **ignoring case**, as long as its left edge opens a word — `statementReducer` carries `reducer`, `APIParser` carries `parser`. The right edge is deliberately free, so a plural or an agent noun still counts (`users` carries `user`); the cost is a name that merely opens with the namespace (`mode` found in `models`), which is the narrower failure than `key` found in `monkey` if the left edge were free too. `namespace.HasPrefix` remains as the stricter both-edges test for the exact shape the fix produces; `Qualify` spells the first word of the name the way Go spells an initialism (`id` → `userID`) and no-ops on a name that already carries the namespace.
