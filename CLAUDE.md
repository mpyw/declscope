...
- **A type is judged with its members.** Narrowing a type narrows every member that states no scope, through containment. So a type is reported only when each such member is unexported and unreached as well, and then those members are not reported again: the type's fix settles them. Narrowing a type over a reached member would start a boundary report, and over an exported one would narrow published API.
