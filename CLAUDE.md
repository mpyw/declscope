...
`rules.allowBoundary` is not that shape. It switches one rule off, the way `rules.surplus: off` does, and it exists for the repository that wants the ownership mark in a name without the scope behind it. **It must not be made to switch `surplus` as well.** Turning boundary off does leave `surplus` auditing directives that no longer do a job, and the README says to set `surplus: off` alongside it, but coupling them in code is exactly the key that silences more than its name says. Two keys, each doing what it is called.

`allowBoundary` is also not the way to adopt declscope gradually, and the README says so where it is documented. A baseline records what a codebase already has and still reports what is new; a switch reports nothing, and the cost of turning it on is never measured. Do not recommend it for adoption.
...
internal/
  ...
  mode.go                 QualifyMode and SurplusMode, the vocabularies
                          rules.naming.qualify and rules.surplus are spelled in
...
`internal.SurplusMode` is a sibling of `internal.QualifyMode` rather than three more `QualifyMode` values. `QualifyMode` carries `Applies(namespaces)`, a question about how many namespaces a package has, which means nothing for this rule, and a `QualifyMode` that could hold `loose` would still need an answer to it. **Each mode type is named for the rule it belongs to.** A bare `Mode` read as a shared vocabulary while holding `ondemand`, which only the naming rule means. The shape is the same (`SurplusModeSet.Parse`, the accepted-values error, `String()` round-tripping the config spelling), and the part the two share, spelling "a, b or c", is `modeJoin`.
