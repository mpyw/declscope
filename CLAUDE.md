...
- `testdata/src/*` are `analysistest` packages. Several carry their own `.declscope.yaml` (`qualifyalways/`, `corens/`, and the `fix*` packages that need a non-default rule), which also exercises config discovery end to end.
