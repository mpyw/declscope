...
- **The chain puts back what a merge hid.** `Decl.Merge` keeps the block's directive in `Beneath` when a spec states its own. Without it, a spec's `private` under a `package` block is judged against the file, and deleting it widens the spec.
- **`loose` still reads the merged chain.** Its test does not see a block's directive beneath a spec's own, so a spec's `//declscope:private` under a `//declscope:package` block, under a file-level `//declscope:private`, is reported unused although deleting it widens the spec. The same happens to an exported spec's `//declscope:package` under a `//declscope:private` block. It was left alone because `rules.directive` was added under the rule that `loose` must not change. Fixing it means passing the full chain to `isInertScopeSite`, and it removes reports.
...
- **The fix is withheld where the deletion would move another report**, with the report kept: a taker of the directive's scope crossing (the boundary message names the level that decided), a field whose type the boundary fix widens in this run (`widened`, passed from `report`), a `//declscope:package` restating an enclosing one while `surplus` reads it (the declaration would join that directive's dependents, which `surplus: strict` judges), one under an ignore covering `surplus`, and one whose next level out is a directive that keeps its own report. Each has a convergence case, and removing any of the first four breaks it. The last is defensive: an ignore naming `directive` is never reported unused, so no case shows it today.
