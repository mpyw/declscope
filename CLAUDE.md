...
**A receiver-ignoring method is a way to dodge the naming rules, and golangci-lint closes it.** Methods are exempt from `qualify` because a method is never read bare. Writing a free helper as a method whose body never names its receiver takes that exemption without earning it, and it happened here once already. `revive`'s `unused-receiver` is enabled for exactly this; declscope has no business knowing about the shape. Do not add a receiver to silence a prefix.
